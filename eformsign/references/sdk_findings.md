...
## POST /api/documents/external — `recipients` Required (Legacy Key Typo)

**Finding**: `send_external_pdf` alone does **not** satisfy the server's requirement. The server also expects a `recipients` array (or the legacy typo key `receipients`). Without it, the document may not be created or sent correctly.

**Legacy typo key**: The eformsign API internally uses `receipients` (missing 'i') as a key in some legacy endpoints. Both `recipients` and `receipients` should be included in the body with identical values to ensure compatibility across API versions.
...
```json
{
  "document": {
    "recipients": [
      { "step_type": "03", "name": "Hong Gildong", "email": "recipient@example.com", "use_mail": true }
    ],
    "receipients": [
      { "step_type": "03", "name": "Hong Gildong", "email": "recipient@example.com", "use_mail": true }
    ],
...
**`step_type` for external recipients:**
- `"03"` — legacy external recipient: requires `name` + `email` (or `sms`)
- `"05"` — new-style participant: requires `member.name` + `member.id`

**Required notification field**: `use_mail: true` or `use_sms: true` on each recipient entry — omitting both returns error **4000001**.

---

## DELETE /api/documents — `comment` Required

**Finding**: The Swagger spec lists only `document_ids` in the request body, but the server treats `comment` as a required property. Omitting `comment` causes an error even though the spec marks it as optional.
...
Send an empty string `""` when no deletion reason is needed. The MCP server automatically includes `comment` (defaulting to `""`) to handle this.

---

## POST /api/documents/{id}/external_decline — API Key Base64 Bearer Auth Required

**Finding**: The official docs do not specify the authentication method for this endpoint, but it requires the same **API Key Base64 Bearer** auth as `POST /api/documents/external` (create_external). Using a regular Access Token returns error **4030039** ("The apiKey encoded has an error").

**Auth header:**
```
Authorization: Bearer {Base64(API_KEY_ID)}
```

**Endpoints that require API Key Base64 Bearer (not Access Token):**
| Endpoint | Note |
|----------|------|
| `POST /api/documents/external` | create_external — documented |
| `POST /api/documents/{id}/external_decline` | confirmed via 4030039 error on Access Token |
